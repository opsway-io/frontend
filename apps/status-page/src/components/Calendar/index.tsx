import { Table, Tooltip, Typography, Box, TablePagination, TableBody, TableCell, TableHead, TableRow, IconButton, Stack, Divider } from "@mui/material";
...
const Calendar: FunctionComponent<CalendarProps> = ({ maintenanceEvents = [] }) => {
  ...
  const generateCalendar = (y: number, m: number) => {
    ...
    while (weeks[currentWeek] && weeks[currentWeek].length < 7 && weeks[currentWeek].length > 0) {
      weeks[currentWeek].push(undefined);
    }
  ...
  return (
    <Box>
      <Stack direction="row" alignItems="center" justifyContent="space-between" sx={{ px: 2, py: 1.5, borderBottom: "1px solid rgba(255,255,255,0.08)" }}>
            ...
            {m.map((d, dIdx) => {
              ...
              return (
                <td 
                  ...
                  style={{
                    backgroundColor: bg,
                    color: hasMaintenance || isSelected ? "#60a5fa" : "inherit",
                    fontWeight: hasMaintenance || isSelected ? "bold" : "normal",
                    padding: 0,
                ...
                >
                  <Tooltip 
                    title={
                      hasMaintenance ? (
                        <Box sx={{ p: 1 }}>
                          <Typography variant="subtitle2" gutterBottom>Maintenance Events:</Typography>
                          {dayMaintenance.map((m, i) => (
                            <Box key={m.id} mb={i < dayMaintenance.length - 1 ? 2 : 0}>
                              <Typography variant="body2" fontWeight="bold">{m.title}</Typography>
                              <Typography variant="caption">{new Date(m.startAt).toLocaleTimeString()} - {new Date(m.endAt).toLocaleTimeString()}</Typography>
                            </Box>
                          ))}
                        </Box>
                      ) : ""
    ...
    <Box sx={{ mt: 3, px: 2 }}>
      <Typography variant="h6" gutterBottom>
        {selectedDate ? `Maintenance for ${new Date(year, month, selectedDate).toLocaleDateString()}` : "All Maintenance Events"}
      </Typography>
      ...
      <Table>
        <TableHead>
          <TableRow>
            <TableCell sx={{ color: "text.secondary", fontWeight: "600", borderColor: "rgba(255,255,255,0.08)" }}>Title</TableCell>
            <TableCell sx={{ color: "text.secondary", fontWeight: "600", borderColor: "rgba(255,255,255,0.08)" }}>Start Time</TableCell>
            <TableCell sx={{ color: "text.secondary", fontWeight: "600", borderColor: "rgba(255,255,255,0.08)" }}>End Time</TableCell>
          </TableRow>
        </TableHead>
        <TableBody>
          {paginatedEvents.length > 0 ? (
            paginatedEvents.map((event) => (
              <TableRow key={event.id} sx={{ '&:last-child td, &:last-child th': { border: 0 } }}>
                ...
                <TableCell sx={{ borderColor: "rgba(255,255,255,0.05)" }}>{new Date(event.startAt).toLocaleString()}</TableCell>
                <TableCell sx={{ borderColor: "rgba(255,255,255,0.05)" }}>{new Date(event.endAt).toLocaleString()}</TableCell>
              </TableRow>
            ))
          ) : (
            <TableRow>
              <TableCell colSpan={3} align="center" sx={{ py: 4, borderColor: "rgba(255,255,255,0.05)" }}>
